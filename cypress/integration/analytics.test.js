    ...
    it('adds add_to_cart event in the data layer at quantity increase in the minicart', () => {
      ...
      cy.getById('buy-button')
        ...
        .then(($btn) => {
          ...
          cy.get(
            '[data-testid=cart-item] [data-testid=fs-quantity-selector-right]'
          )
            .click()
            .then(() => {
              cy.itemsInCart(2)
...
describe('remove_from_cart event', () => {
  ...
  context('when decreasing product quantity', () => {
    it('adds remove_from_cart event in the data layer at quantity decrease in the minicart', () => {
      ...
      // Add item to cart
      cy.get('[data-testid=fs-quantity-selector-right]')
        .click()
        .then(() => {
          cy.getById('buy-button')
            .should('be.visible')
            ...
            .then(($btn) => {
              ...
              // Remove the added item
              cy.get(
                '[data-testid=cart-item] [data-testid=fs-quantity-selector-left]'
              )
                .click()
                .then(() => {
                  cy.itemsInCart(1)
