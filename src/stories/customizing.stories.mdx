...
To build your new theme, just create a new `.scss` file on `src/styles/global` and overwrite the tokens you want. We created two themes to use as examples: `theme-soft-blue` and `theme-midnight`.

FastStore uses a default theme called `Brandless`, which provides structural styles for your components and pages. Please read <a href="../?path=/docs/theming-architecture-naming--page">Architecture & Naming's page</a> to further information about our themeable architecture.
