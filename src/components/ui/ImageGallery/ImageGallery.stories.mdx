...
export const TemplateWithoutSelector = (args) => {
  const productImages = [
    {
      url: 'https://storeframework.vtexassets.com/arquivos/ids/190902/unsplash-magic-mouse.jpg?v=637800136963870000',
      alternateName: 'Magicwhite',
...
<TokenTable>
  <TokenRow
    token="--fs-image-gallery-selector-thumbnail-width"
    value="#{rem(56px)}"
  />
  <TokenRow
  ...
  />
  <TokenRow
    token="--fs-image-gallery-selector-thumbnail-border-color-selected"
