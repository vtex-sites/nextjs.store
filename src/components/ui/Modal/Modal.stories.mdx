import { Meta, Canvas, Story, ArgsTable } from '@storybook/addon-docs'
import UIProvider, { useUI } from 'src/sdk/ui/Provider'
import Button from 'src/components/ui/Button'
import { RegionalizationModalContent } from 'src/components/regionalization/RegionalizationModal'
import {  SessionProvider } from '@faststore/sdk'

import Modal from '.'
import {
  TokenTable,
  TokenRow,
  TokenDivider,
} from 'src/../.storybook/components'

<Meta
  title="Molecules/Modal"
  component={Modal}
/>

export const ModalTemplate = (args) => {
  const { modal, openModal } = useUI()
  return (
    <div>
      <Button variant="primary" onClick={() => openModal()}>Open Modal</Button>
      {
        modal && (
          <Modal>
            {({ fadeOut }) => (
              <div style={{
                  display: "flex",
                  alignItems: "center",
                  flexDirection: "column",
                  padding: "3rem",
                  backgroundColor: "white",
              }}>
                <h1 style={{
                  fontSize: "2rem",
                  paddingBottom: "1rem",
                }}>
                  Hello, i'm a Modal!
                </h1>
                <Button onClick={fadeOut}>Close Modal</Button>
              </div>
            )}
          </Modal>
        )
      }
    </div>
  )
}

export const Template = (args) => {
  return (
    <UIProvider>
      <ModalTemplate />
    </UIProvider>
  )
}

<header>

# Modal

Modals are dialog windows that sit on top of an application's main view. Once a Modal is open, interactions with the content behind it are blocked.

</header>

- [Overview](#overview)
- [Usage](#usage)
- [Related Components](#related-components)

## Overview

The `Modal` component uses [FastStore UI Modal](https://www.faststore.dev/reference/ui/molecules/Modal) as base.

---

## Usage

`import Modal from 'src/components/ui/Modal'`

<Canvas>
  <Story
    name="Overview"
  >
    {Template.bind({})}
  </Story>
</Canvas>

---

<TokenTable>
  <TokenRow
    token="--fs-modal-position-top"
    value="30vh"
  />
  <TokenRow
    token="--fs-modal-position-right"
    value="var(--fs-spacing-4)"
  />
  <TokenRow
    token="--fs-modal-position-left"
    value="var(--fs-spacing-4)"
  />
  <TokenRow
    token="--fs-modal-max-width"
    value="calc(var(--fs-grid-breakpoint-desktop) / 3)"
  />
  <TokenRow
    token="--fs-modal-min-height"
    value="30px"
  />
  <TokenRow
    token="--fs-modal-margin"
    value="auto"
  />
  <TokenRow
    token="--fs-modal-border-radius"
    value="var(--fs-spacing-0)"
  />
</TokenTable>

### Tablet

<TokenTable>
    <TokenRow
    token="--fs-modal-width-tablet"
    value="calc(100vw / 3)"
    isColor
  />
  <TokenRow
    token="--fs-modal-min-width-tablet"
    value="calc(var(--fs-grid-breakpoint-desktop) / 3)"
  />
</TokenTable>


## Transition

### In

<TokenTable>
    <TokenRow
      token="--fs-modal-transition-state-in-timing"
      value="0.2s"
    />
    <TokenRow
      token="--fs-modal-transition-state-in-property"
      value="transform"
    />
    <TokenRow
      token="--fs-modal-transition-state-in-function"
      value="ease-in"
    />
</TokenTable>

### Out

<TokenTable>
    <TokenRow
      token="--fs-modal-transition-state-out-timing"
      value="0.2s"
    />
    <TokenRow
      token="--fs-modal-transition-state-out-property"
      value="transform"
    />
    <TokenRow
      token="--fs-modal-transition-state-out-function"
      value="ease-in"
    />
</TokenTable>

---

## Related Components

<section className="sbdocs-section">
  <article>
    <div style={{ height: '280px' }}>
      <SessionProvider>
        <RegionalizationModalContent />
      </SessionProvider>
    </div>
    <a href="?path=/docs/features-regionalization-regionalizationmodal--overview">
      <article className="sbdocs-section-text">
          <h3>Regionalization Modal Content</h3>
          <p>
            Displays a set of actions/items to the user, usually used to show a menu of options.
          </p>
      </article>
    </a>
  </article>
</section>

---
