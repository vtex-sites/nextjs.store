import { Meta, Canvas, Story, ArgsTable } from '@storybook/addon-docs'
import {
  TokenTable,
  TokenRow,
  TokenDivider,
} from 'src/../.storybook/components'
import ProductCardSkeleton from '.'

<Meta
  component={ProductCardSkeleton}
  title="Features/Skeletons/ProductCardSkeleton"
  argTypes={{
    bordered: {
      control: 'boolean',
      table: { type: 'boolean' },
    },
    sectioned: {
      control: 'boolean',
      table: { type: 'boolean' },
    },
    displayButton: {
      control: 'boolean',
      table: { type: 'boolean' },
    },
  }}
/>

export const TemplateProductCardSkeleton = ({ ...args }) => (
  <ProductCardSkeleton {...args} />
)

<header>

# Product Card Skeleton

The loading placeholder for the <code>ProductCard</code> used in several sections of the store.

</header>

## Overview

The `ProductCardSkeleton` component is part of [Skeletons](?path=/docs/features-skeletons-overview--page) feature.

---

## Usage

`import ProductCardSkeleton from 'src/components/skeletons/ProductCardSkeleton'`

<Canvas
  className="no-code canvas-product-card-skeleton"
  style={{ backgroundColor: '#f7f7f7' }}
>
  <Story
    name="Usage"
    args={{
      bordered: false,
      sectioned: false,
      variant: 'default',
      displayButton: false,
    }}
  >
    {TemplateProductCardSkeleton.bind({})}
  </Story>
</Canvas>

<ArgsTable story="Usage" />

<TokenTable>
  <TokenRow token="--fs-product-card-skeleton-min-width-mobile" value="14vw" />
  <TokenRow
    token="--fs-product-card-skeleton-min-width-desktop"
    value="12rem"
  />
  <TokenRow token="--fs-product-card-skeleton-height" value="100%" />
  <TokenRow
    token="--fs-product-card-skeleton-padding"
    value="var(--fs-spacing-1) var(--fs-spacing-1) var(--fs-spacing-2)"
  />
  <TokenRow
    token="--fs-product-card-skeleton-border"
    value="var(--fs-border-width) solid transparent"
  />
  <TokenRow
    token="--fs-product-card-skeleton-border-radius"
    value="var(--fs-border-radius)"
  />
</TokenTable>

### Content

<TokenTable>
  <TokenRow token="--fs-product-card-skeleton-content-height" value="20%" />
  <TokenRow
    token="--fs-product-card-skeleton-content-margin-top"
    value="var(--fs-spacing-2)"
  />
  <TokenRow
    token="--fs-product-card-skeleton-content-button-margin-top"
    value="var(--fs-spacing-4)"
  />
  <TokenRow
    token="--fs-product-card-skeleton-content-text-margin-bottom"
    value="var(--fs-product-card-skeleton-content-margin-top)"
  />
  <TokenRow
    token="--fs-product-card-skeleton-content-badge-min-height"
    value="var(--fs-spacing-5)"
  />
  <TokenRow
    token="--fs-product-card-skeleton-content-badge-margin-top"
    value="var(--fs-product-card-skeleton-content-text-margin-top)"
  />
  <TokenRow
    token="--fs-product-card-skeleton-content-badge-max-width"
    value="var(--fs-product-card-skeleton-content-height)"
  />
  <TokenRow
    token="--fs-product-card-skeleton-content-badge-min-width-mobile"
    value="30%"
  />
  <TokenRow
    token="--fs-product-card-skeleton-content-badge-min-width-tablet"
    value="var(--fs-product-card-skeleton-content-height)"
  />
</TokenTable>

---

## Variants

### Sectioned

<Canvas
  className="no-code canvas-product-card-skeleton"
  style={{ backgroundColor: '#f7f7f7' }}
>
  <Story name="Sectioned" args={{ sectioned: true }}>
    {TemplateProductCardSkeleton.bind({})}
  </Story>
</Canvas>

<TokenTable>
  <TokenRow
    token="--fs-product-card-skeleton-sectioned-image-min-width"
    value="13rem"
  />
  <TokenRow
    token="--fs-product-card-skeleton-sectioned-image-height"
    value="var(--fs-product-card-skeleton-image-height-desktop)"
  />
</TokenTable>

### Bordered

<Canvas
  className="no-code canvas-product-card-skeleton"
  style={{ backgroundColor: '#f7f7f7' }}
>
  <Story name="Bordered" args={{ bordered: true }}>
    {TemplateProductCardSkeleton.bind({})}
  </Story>
</Canvas>

<TokenTable>
  <TokenRow token="--fs-product-card-skeleton-bordered-mobile" value="none" />
  <TokenRow
    token="--fs-product-card-skeleton-bordered-desktop"
    value="var(--fs-border-width) solid var(--fs-border-color-light)"
  />
</TokenTable>
