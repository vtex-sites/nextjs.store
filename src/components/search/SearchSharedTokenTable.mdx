import {
  TokenTable,
  TokenRow,
  TokenDivider,
} from 'src/../.storybook/components'

*This tokens are shared between
[SearchDropdown](?path=/docs/features-search-searchdropdown--default-story),
[SearchHistory](?path=/docs/features-search-searchhistory--default-story),
[SearchInput](?path=/docs/features-search-searchinput--default-story),
[SearchSuggestions](?path=/docs/features-search-searchsuggestions--default-story),
[SearchTop](?path=/docs/features-search-searchtop--default-story).

<TokenTable>
  <TokenRow
    token="--fs-search-min-height"
    value="var(--fs-spacing-7)"
  />
</TokenTable>

### Title
<TokenTable>
  <TokenRow
    token="--fs-search-title"
    value="var(--fs-text-size-3)"
  />
  <TokenRow
    token="--fs-search-title-line-height"
    value="1.5"
  />
  <TokenDivider />
  <TokenRow
    token="--fs-search-title-small-margin-bottom"
    value="var(--fs-spacing-1)"
  />
  <TokenRow
    token="--fs-search-title-small-font-size"
    value="var(--fs-text-size-2)"
  />
  <TokenRow
    token="--fs-search-title-small-color"
    value="var(--fs-color-neutral-6)"
  />
</TokenTable>

### Section
<TokenTable>
  <TokenRow
    token="--fs-search-section-padding"
    value="var(--fs-spacing-1) 0"
  />
  <TokenRow
    token="--fs-search-section-background-color"
    value="var(--fs-color-neutral-0)"
  />
  <TokenRow
    token="--fs-search-section-border-bottom"
    value="var(--fs-border-width) solid var(--fs-border-color-light)"
  />
</TokenTable>

### Link
<TokenTable>
  <TokenRow
    token="--fs-search-item-link-color "
    value="var(--fs-color-text-light)"
  />
  <TokenRow
    token="--fs-search-item-link-width"
    value="100%"
  />
  <TokenRow
    token="--fs-search-item-link-padding-right"
    value="var(--fs-spacing-3)"
  />
  <TokenRow
    token="--fs-search-item-link-padding-left"
    value="var(--fs-spacing-3)"
  />
</TokenTable>

### Item
<TokenTable>
  <TokenRow
    token="--fs-search-item-margin-right"
    value="calc(-1 * var(--fs-spacing-3))"
  />
  <TokenRow
    token="--fs-search-item-margin-left"
    value="calc(-1 * var(--fs-spacing-3))"
  />
  <TokenRow
    token="--fs-search-item-font-size"
    value="var(--fs-text-size-2)"
  />
  <TokenRow
    token="--fs-search-item-line-height"
    value="1.25"
  />
  <TokenRow
    token="--fs-search-item-background-color-hover"
    value="var(--fs-color-main-0)"
  />
</TokenTable>

### Item Bold
<TokenTable>
  <TokenRow
    token="--fs-search-item-bold-font-weight"
    value="var(--fs-text-weight-bold)"
  />
  <TokenRow
    token="--fs-search-item-bold-color"
    value="var(--fs-text-weight-bold)"
  />
</TokenTable>

### Icon
<TokenTable>
  <TokenRow
    token="--fs-search-item-icon-color"
    value="var(--fs-color-neutral-4)"
  />
  <TokenRow
    token="--fs-search-item-icon-margin-right"
    value="var(--fs-spacing-1)"
  />
</TokenTable>

### Badge
<TokenTable>
  <TokenRow
    token="--fs-search-badge-margin-right"
    value="var(--fs-spacing-1)"
  />
</TokenTable>

